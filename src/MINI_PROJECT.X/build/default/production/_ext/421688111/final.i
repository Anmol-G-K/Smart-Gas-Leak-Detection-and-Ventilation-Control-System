# 1 "C:/Users/GNKRISHNAN/MPLABXProjects/MINI_PROJECT.X/final.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "C:/Users/GNKRISHNAN/MPLABXProjects/MINI_PROJECT.X/final.c" 2
...
# 1 "C:\\Users\\GNKRISHNAN\\Documents\\CODING\\23EEE214_MCA\\HEADERS\\lcd.h" 1
# 30 "C:\\Users\\GNKRISHNAN\\Documents\\CODING\\23EEE214_MCA\\HEADERS\\lcd.h"
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.50\\pic\\include\\c99\\stdbool.h" 1 3
# 30 "C:\\Users\\GNKRISHNAN\\Documents\\CODING\\23EEE214_MCA\\HEADERS\\lcd.h" 2
...
# 2 "C:/Users/GNKRISHNAN/MPLABXProjects/MINI_PROJECT.X/final.c" 2
# 24 "C:/Users/GNKRISHNAN/MPLABXProjects/MINI_PROJECT.X/final.c"
const unsigned int ppm_lookup[10] = {10, 20, 35, 50, 75, 100, 150, 200, 300, 500};


void init_system();
unsigned int read_adc(unsigned char channel);
unsigned int calculate_lpg_ppm(unsigned int rs);
...
unsigned int gas_alert_counter = 0;
...
void main() {
    ...
    Lcd_Clear();
    Lcd_Set_Cursor(1,1);
    Lcd_Print_String("Gas Level:");

    while(1) {
        gas_value = read_adc(0);
        check_gas_level(gas_value);

        unsigned int LPG_ppm = calculate_lpg_ppm(gas_value);
        Lcd_Set_Cursor(1,11);
        Lcd_Print_String("    ");
        Lcd_Set_Cursor(1,11);
        Lcd_Print_Number(LPG_ppm);
...
unsigned int calculate_lpg_ppm(unsigned int adc_value) {
    unsigned int voltage_mv = (adc_value * 5000) / 1023;
    unsigned int rs = ((5000 - voltage_mv) * 1000) / voltage_mv;
    unsigned int rs_ro_ratio = rs / 10;
    if (rs_ro_ratio < 10) return ppm_lookup[0];
    else if (rs_ro_ratio < 20) return ppm_lookup[1];
    else if (rs_ro_ratio < 35) return ppm_lookup[2];
    else if (rs_ro_ratio < 50) return ppm_lookup[3];
    else if (rs_ro_ratio < 75) return ppm_lookup[4];
    else if (rs_ro_ratio < 100) return ppm_lookup[5];
    else if (rs_ro_ratio < 150) return ppm_lookup[6];
    else if (rs_ro_ratio < 200) return ppm_lookup[7];
    else if (rs_ro_ratio < 300) return ppm_lookup[8];
    else return ppm_lookup[9];
}
...
void check_gas_level(unsigned int gas_value) {
    if (gas_value > 200) {
        PORTCbits.RC0 = 1;
        gas_alert_counter++;
        if (gas_alert_counter >= 5) {
            pwm_set_duty((gas_value * 1023) / 1023);
        }
        _delay((unsigned long)((200)*(20000000/4000.0)));
        Lcd_Set_Cursor(2,1);
        Lcd_Print_String("Gas Detected!");
        PORTCbits.RC0 = 0;
        _delay((unsigned long)((200)*(20000000/4000.0)));
    } else {
        PORTCbits.RC0 = 0;
...
void delay_ms(unsigned int ms) {
    while(ms--) _delay((unsigned long)((1)*(20000000/4000.0)));
}
